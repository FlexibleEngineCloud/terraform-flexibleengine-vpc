...
locals {
  ...
  # vpc_subnets_snat_cidr = flatten([for x in var.vpc_snat_subnets : [for y in var.vpc_subnets: y.subnet_cidr if y.subnet_name == x ]])
  vpc_subnets_snat_cidr = flatten([for x in var.vpc_subnets: x.subnet_cidr if (x.subnet_is_nat != null && x.subnet_is_nat) ])
